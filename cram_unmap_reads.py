...
def count_unmapped_reads(sample, cram_path, region, seqs):
    pattern = "|".join(seqs)
    try:
        cmd = f"samtools view -f 4 {cram_path} | cut -f10 | grep -E -c '{pattern}'"
...
with ThreadPoolExecutor(max_workers=MAX_THREADS) as executor:
    ...
    for sample, cram in sample_crams.items():
        for region in amplicons:
            ...
            futures.append(executor.submit(count_unmapped_reads, sample, cram, region, seqs))
...
pivot.to_csv("amplicon_cram_unmapped_counts.tsv", sep="\t")

print("amplicon_cram_unmapped_counts.tsv")
